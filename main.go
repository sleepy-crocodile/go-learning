...
func main() {
	...
	exercise.PrintName("")
	exercise.PrintName("John Doe")

	fmt.Println("\nExercise 2: Print name and age")
	exercise.PrintNameAndAge("Jane Doe", 30)

	fmt.Println("\nExercise 3: Print numbers from 1 to 10")
	exercise.PrintNumbersUsingLoop(10, false)
	...
	exercise.PrintNumbersUsingLoop(10, true)

	fmt.Println("\nExercise 6: Generate random number in range [1, 10]")
	fmt.Println(exercise.GenerateRandomInRange(1, 10))

	fmt.Println("\nExercise 7: Print current date")
	fmt.Println(exercise.PrintCurrentDate())
}
